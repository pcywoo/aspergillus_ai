...
ax[1].set_title(
    't-SNE scattered plot of fungal images through trained Densenet neural networks')
...
ax3.set_title('Heatmap of actual classification of fungi to\n predict of pictures of fungi through Densenet neural network')
font={'style':'italic','horizontalalignment':'center'}
font1={'style':'italic','horizontalalignment':'center'}
plt.setp(ax3.get_xticklabels(),**font)
plt.setp(ax3.get_yticklabels(),**font1)
...
ax10left, ax10right = ax1[0].get_xlim()
twinax0 = ax1[0].twiny()
...
ax11left, ax11right = ax1[1].get_xlim()
twinax1 = ax1[1].twiny()
twinax0.set_xlim(ax10left/176, ax10right/176)
twinax1.set_xlim(ax11left/176, ax11right/176)
twinax0.set_xticklabels('{:,.0f}'.format(x) for x in twinax0.get_xticks())
twinax1.set_xticklabels('{:,.0f}'.format(x) for x in twinax1.get_xticks())
