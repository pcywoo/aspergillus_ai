...
%% Load Data
%% Load Data
results_folder = 'E:\OneDrive - The University Of Hong Kong\Postdoc\5_Paper\Results_aug_20200918\res50';
...
for i =1:length(classList)
    ...
    specie=char(classList(i));
%     specieList(i)=specie(1:3);
    specieNum=specie(1:3)+0;
%     colors(i,1)=specieNum(1)/sum(specieNum);
%     colors(i,2)=specieNum(2)/sum(specieNum);
%     colors(i,3)=specieNum(3)/sum(specieNum);
%     colors(i,1)=specieNum(1)/256;
%     colors(i,2)=specieNum(2)/256;
%     colors(i,3)=specieNum(3)/256;
    colors(i,1)=abs(specieNum(1)-mean(specieNum))/(max(specieNum)-min(specieNum));
    colors(i,2)=abs(specieNum(2)-mean(specieNum))/(max(specieNum)-min(specieNum));
    colors(i,3)=abs(specieNum(3)-mean(specieNum))/(max(specieNum)-min(specieNum));
end
